...
class SimpleHT:
  ...
  #to check whther the server is active or not
  def active(self):
  ...
  def get(self, path, server_table):
    ...
    if server_table == '0':
      ...
      print("The value of dataserver2 is taken from dataserver0")
      time.sleep(0)
      return Binary(pickle.dumps((datafile2[path])))
    elif server_table == '3':
      print("The value of dataserver3 is taken from dataserver0")
      time.sleep(0)
  ...
  def put(self, path, server_table, index, value):
    ...
    if server_table == "0":
      ...
      variable.sync()
      return Binary(pickle.dumps(True)) 
    elif server_table == "0-0-NULL":
      datafile0[path] = {}
      variable['0'] = datafile0
      variable.sync()
      return Binary(pickle.dumps(True))
    elif server_table == "0-2-NULL":
      datafile2[path] = {}
      variable['2'] = datafile2
      variable.sync()
      return Binary(pickle.dumps(True))
    elif server_table == "0-3-NULL":
      datafile3[path] == {}
      variable['3'] = datafile3
      variable.sync()
      return Binary(pickle.dumps(True))
...
def main(port_number):
  print("***************************************dataserver0 is running***********************************************", int(port_number))
...
if __name__ == "__main__":
  if len(argv) != 5:
    ...
    port = 0000
    variable = shelve.open("dataserver0.dat")
